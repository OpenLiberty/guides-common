...
If the server is still running from the previous steps, stop it first using the Maven liberty:stop-server goal from command line in the `start` directory:
