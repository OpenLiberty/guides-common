...
status_7=0
############################################################################################
echo -e "\033[1;32m[8] Building the application ... \033[0m"
cd finish
mvnOutput=$(mvn clean install)
if [ $? -eq 0 ]; then
  status_8=0
else
  status_8=1
  echo "$mvnOutput" | grep -w --color=always "\<ERROR\>"
fi
echo -e "\n"
############################################################################################
echo -e "\033[1;32m_______________[ SUMMARY ]_______________\033[0m"
statuses=($status_1 $status_2 $status_3 $status_4 $status_5 $status_6 $status_7 $status_8)
for ((i = 0; i < ${#statuses[@]}; ++i)); do
  number=$(($i+1))
  if [ ${statuses[$i]} -eq 0 ] ; then
    echo -e "\033[1;32m|              [$number] Passed               |\033[0m"
  elif [[ $i -eq 7 ]] && [[ ${statuses[$i]} -eq 1 ]]; then
    echo -e "\033[1;31m|              [$number] ERROR                |\033[0m"
  else
    echo -e "\033[1;31m|              [$number] Warning              |\033[0m"
  fi
done

if [ $status_8 -eq 1 ]; then
  exit 1
fi
