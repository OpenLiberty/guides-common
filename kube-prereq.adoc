...
Complete the setup for your operating system:
  ...
  - Set up https://docs.docker.com/docker-for-windows/#kubernetes[Docker for Windows^]. In the Docker Preferences, ensure that the `Expose daemon on tcp://localhost:2375 without TLS` option is enabled in the _General_ tab. This setting is required when the `dockerfile-maven` plugin is used in the POM file.
